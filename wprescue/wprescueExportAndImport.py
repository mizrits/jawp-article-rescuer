...
def importxmlandedit(title,titlee,fid: str):
    S = requests.Session()
    ...
    #### import
    summary=f"ボットによる自動インポート: ウィキペディア日本語版 [[jawp:{title}|https://ja.wikipedia.org/wiki/{title}]] から全版をインポートしました"
...
def ExportAndImport(title):
    ...
    importxmlandedit(title=title,titlee=titlee,fid=fid)
